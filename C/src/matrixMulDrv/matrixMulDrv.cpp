    ...
    // This is the new CUDA 4.0 API for Kernel Parameter passing and Kernel Launching (advanced method)
        ...
        *((CUdeviceptr *)&argBuffer[offset]) = d_B;   offset += sizeof(d_B);
	    ...
	    if (use_64bit_memory_address && (totalGlobalMem > (unsigned long long)4*1024*1024*1024L)) {
            ...
            *((CUdeviceptr *)&argBuffer[offset]) = Matrix_Width_A;  offset += sizeof(Matrix_Width_A);
            *((CUdeviceptr *)&argBuffer[offset]) = Matrix_Width_B;  offset += sizeof(Matrix_Width_B);
	    } else {
		    int Matrix_Width_A = WA;
		    int Matrix_Width_B = WB;

            *((int *)&argBuffer[offset]) = Matrix_Width_A;  offset += sizeof(Matrix_Width_A);
            *((int *)&argBuffer[offset]) = Matrix_Width_B;  offset += sizeof(Matrix_Width_B);
	    }
...
initCUDA(int argc, char **argv, CUfunction *pMatrixMul, int *block_size_out )
{
    ...
    *block_size_out = block_size;
    ...
    cutilDrvSafeCallNoSync( cuDeviceTotalMem(&totalGlobalMem, cuDevice) );
