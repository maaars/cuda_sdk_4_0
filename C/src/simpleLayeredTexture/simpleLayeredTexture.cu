...
main( int argc, char** argv) 
{
    ...
    cutilSafeCall( cudaMalloc3DArray( &cu_3darray, &channelDesc, make_cudaExtent(width, height, num_layers), cudaArrayLayered )); 
    ...
    cutilSafeCall( cudaMemcpy3D( &myparms));
    ...
    cutilSafeCall( cudaBindTextureToArray( tex, cu_3darray, channelDesc));
    ...
    cutilCheckMsg("warmup Kernel execution failed");
    ...
    printf("%.2f Mtexlookups/sec\n", (width*height*num_layers / (cutGetTimerValue( timer) / 1000.0f) / 1e6));
...
#define MIN_EPSILON_ERROR 5e-3f
        ...
        bResult = (bool)cutComparefe( h_odata, h_data_ref, width*height*num_layers, MIN_EPSILON_ERROR);
    }
    ...
    cutilSafeCall(cudaFreeArray(cu_3darray));
