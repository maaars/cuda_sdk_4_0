...
int main(int argc, const char **argv)
{
    ...
    {
        // Unknown copy engines and caps, but give it a try...
        fMinPassCriteria[0] = PASS_FACTOR * EXPECTED_OVERLAP;                // 1st cycle
        fMinPassCriteria[1] = PASS_FACTOR * EXPECTED_OVERLAP;                // avg of many cycles
    }
    ...
    // Create 2 command-queues and explicitly turn off out-of-order in case an OpenCL implementation supports it (don't want OOO for this code)
    cqCommandQueue[0] = clCreateCommandQueue(cxGPUContext, cdDevices[uiTargetDevice], CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &ciErrNum);
    oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    shrLog("clCreateCommandQueue [0]...\n"); 
    cqCommandQueue[1] = clCreateCommandQueue(cxGPUContext, cdDevices[uiTargetDevice], CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &ciErrNum);
    oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
